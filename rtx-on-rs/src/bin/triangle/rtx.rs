...
impl RTXData {
    pub fn new(context: &Arc<Context>, swapchain: &Swapchain, camera: Camera) -> Self {
        ...
        let shader_binding_table_buffer = create_shader_binding_table(context, pipeline, rt_props);
...
fn build_acceleration_structures(
    ...
    let bottom_mem_requirements = bottom_as
        .get_memory_requirements(vk::AccelerationStructureMemoryRequirementsTypeNV::BUILD_SCRATCH);
    let top_mem_requirements = top_as
        .get_memory_requirements(vk::AccelerationStructureMemoryRequirementsTypeNV::BUILD_SCRATCH);
